...
if ( isset($_POST['regNo']) ){
  $regNo = $_POST['regNo'];
  $sql = $connDb->query("SELECT * FROM studs") or die($connDb->error);
  if (!$sql ) {
    echo "A probrem Occured";
  }else {
    if( $sql1 = $connDb->query( "SELECT * FROM studs WHERE reg = '$regNo' " ) ){
      ...
      echo "<table class='table'>";
        echo "<thead>";
          ...
          echo "<th>Amount</th>";
          $sql2 = $connDb->query( "SELECT * FROM studs WHERE reg = '$regNo' " );
