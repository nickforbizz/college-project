...
if (isset($_POST['studentReceipt']) ) {
      ...
      // Name Header
      $getName = $connDb->query("SELECT * FROM studs WHERE reg ='$regNo1'");
      ...
      while ($c = $getName->fetch_assoc()) {
        ...
        // Fetch Name
        $pdf->Cell(40,10,$c['firstName']." ".$c['lastName'],0,1,'L');
        $pdf->Cell(0,0,'',0,1);

        // Reg number
        $pdf->Cell(40,10,'Regestration Number: ',0,0,'L');
        $pdf->Cell(40,10,$c['reg'],0,1,'C');
        $pdf->Cell(0,0,'',0,1);

        if ($connDb->query("SELECT totalfees FROM fees_table")) {
        ...
        $pdf->Cell(40,10,$c['paymode'],0,1,'C');
        $pdf->Cell(0,0,'',0,1);
        // date
        $pdf->Cell(40,10,'Date : ',0,0,'L');
        $pdf->Cell(40,10,$c['update_date'],0,1,'C');
